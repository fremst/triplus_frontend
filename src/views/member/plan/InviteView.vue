...
export default {
  ...
  methods:{
    ...
    add(addId){
     //일행 테이블에 존재여부 확인먼저
    axios.get(`http://localhost:8082/triplus/api/member/plan/companion/identify`,{
      headers:{
        'Access-Control-Allow-Origin': '*'
      },
      params:{
      ...
      if(resp.data.result==='success'){ //존재하면
        ...
        axios.post(`http://localhost:8082/triplus/api/member/plan/companion`,joinparam,{
          headers:{
            'Access-Control-Allow-Origin': '*'
          }
        }).then(function(resp){
