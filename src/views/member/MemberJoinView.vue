  ...
  <div class="wrapper">
    ...
    <div class="inner">
      ...
      <div class="addr">
        ...
        <div><InputText type="text" v-model="address"  placeholder="주소" class="addr-txt" /><span>{{addrErr}}</span></div>
        <div><InputText type="text" v-model="detailAddress"  placeholder="상세주소를 입력해주세요" class="detail-txt"/><span>{{detailErr}}</span></div>
      </div>

      <div class="email">
        <div><label>이메일 주소</label></div>
        <div><InputText type="text" v-model="email" placeholder="이메일주소를 입력하세요" class="email-txt"/><Button label="인증번호 받기" class="p-button-outlined" @click="sendMail"  /></div>
        <span>{{emailErr}}</span>
...
.id span, .pwd span , .name span, .radioErr, .phone span, .addr span, .email span, .bdate span {
  color: red;
}



.id, .pwd, .name, .gender, .phone, .addr, .email, .bdate{
  margin-bottom: 15px;
}
.pwd-txt{
  margin-bottom: 10px;
...
.id-txt, .pwd-txt, .pwd2-txt, .name-txt, .phone-txt, .addr-txt, .detail-txt, .email-txt, .bDate-txt, .cert-txt, .bdate-txt{
  width: 430px;
  height: 40px;
