  ...
  <div class="wrapper">
    ...
    <div class="inner">
      <div class="content">
        ...
        <div class="name">
          <span>이름</span><InputText type="text" v-model="name" class="name-txt" /><span>{{ nameErr }}</span>
        </div>

        <div class="phone">
          <span>전화번호</span><InputText type="text" v-model="phone" class="phone-txt" /><span>{{ phoneErr }}</span>
        </div>

        <div class="bdate">
          <span>생년월일</span><InputText type="date" v-model="bdate" readonly="readonly" class="bdate-txt" />
        </div>
        <div class="regdate">
          <span>가입일</span><InputText type="date" v-model="regdate" readonly="readonly" class="regdate-txt" />
        </div>

        <div class="addr">
          <div class="a">
            <span>우편번호</span>
            <InputText type="text" v-model="postcode" class="post-txt" />
            <Button label="우편번호 찾기" class="p-button-outlined" id="btn1" @click="execDaumPostcode" />
          </div>

          <div class="b">
            <span>주소</span><InputText type="text" v-model="address" class="addr-txt" /><span>{{ addrErr }}</span>
          </div>
          <div class="c">
            <span>상세주소</span><InputText type="text" v-model="detailAddress" class="detail-txt" /><span>{{
              detailErr
            }}</span>
          </div>
        </div>
        ...
        <div class="pwd">
          ...
          <div class="e">
            <span>새 비밀번호 확인</span><InputText v-model="pwd2" class="pwd2-txt" type="password" />
          </div>
          {{ pwdErr }}
          <div>
            <Button type="button" label="비밀번호 수정" class="p-button-primary" id="btn3" @click.prevent="newPwd" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    axios
      .get(`http://localhost:8082/triplus/member/mypage/find/${this.id}`, {
        headers: {
  ...
  methods: {
    ...
    update() {
      ...
      if (
        ...
        axios
          .post("http://localhost:8082/triplus/member/mypage/update", joinparam, {
            headers: {
              "Access-Control-Allow-Origin": "*"
            }
          })
    ...
    newPwd() {
      if (this.pwd1 !== this.pwd2) {
        ...
        if (!check.test(this.pwd1)) {
          ...
          axios
            .post("http://localhost:8082/triplus/member/changepwd", joinparam, {
              headers: {
                "Access-Control-Allow-Origin": "*"
              }
            })
