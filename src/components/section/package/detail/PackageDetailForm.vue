  ...
  <div class="box">
    <div>
      <table class="table-reg">
        <tr class="tr-info">
          <td class="td-title">출&emsp;발</td>
          <td class="td-content">{{ sDate }}<br /></td>
        </tr>
        <tr class="tr-info">
          <td class="td-title">도&emsp;착</td>
          <td class="td-content">{{ eDate }}<br /></td>
        </tr>
      </table>
    </div>
    <hr />
        ...
        <tr class="tr-price">
          ...
          <td rowspan="2" class="td-content">
            <InputNumber
              ...
              v-model="adultCnt"
              :min="1"
              :max="Math.min(packageDetails.vacancy, packageDetails.vacancy - childCnt)"
              showButtons
        ...
        <tr class="tr-price">
          ...
          <td rowspan="2" class="td-content">
            <InputNumber
              ...
              v-model="childCnt"
              :min="0"
              :max="Math.min(packageDetails.vacancy, packageDetails.vacancy - adultCnt)"
              showButtons
              buttonLayout="horizontal"
              incrementButtonIcon="pi pi-plus"
              decrementButtonIcon="pi pi-minus"
    ...
    <Button
      label="예약하기"
      @click="
        this.$router.push({ name: 'package-reservation', query: { childCnt: this.childCnt, adultCnt: this.adultCnt } })
      "
    />
  </div>
</template>
  ...
  computed: {
    ...
    sDate() {
      if (this.packageDetails) {
        return this.$getFormattedDate(new Date(this.packageDetails.sdate));
      } else {
        return 0;
      }
    },
