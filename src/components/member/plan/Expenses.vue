  ...
  <div class="main">
    ...
    <div class="board">
      <div class="board-header">
        ...
        <br />
        <div>
          <Button class="p-button-secondary p-button-rounded p-button-sm mr-4" @click="displaySummary = true"
            >지출 통계</Button
          >
          &nbsp;
          <Button class="p-button-secondary p-button-rounded p-button-sm mr-4" @click="displayCalculate = true"
            >정산하기</Button
          >
        </div>
      </div>
      <div class="board-main">
        <div
          v-for="day of scheduleData.getDayCount() + 1"
          :key="day"
          class="daycard"
        ...
        >
          ...
          <!-- 지출 항목들 -->
          <DataTable class="board-table" :value="expenseData[day - 1]" responsiveLayout="scroll" :rows="10">
            ...
            <Column field="payment" header="금액" style="width: 150px">
              <template #body="slotProps">
                <p style="font-size: 20px; font-weight: 500">
                  {{ new Intl.NumberFormat("en-US").format(slotProps.data.payment) }}원
                </p>
              </template>
            </Column>
          ...
          <!-- 비용 추가 -->
          <Button class="violet-btn" style="width: 400px; margin: auto" @click="displayAddExpense = true"
            >비용 추가</Button
          >
        </div>
        ...
        <!-- 비용 추가 다이얼로그 -->
        <Dialog
          class="dialog-addexpense"
          header="비용 추가하기"
          v-model:visible="displayAddExpense"
          :modal="true"
          style="width: 510px"
        >
          <div>
            ...
            <Divider />
            <div>
              <DataTable class="add-table" :value="scheduleData.users" responsiveLayout="scroll" :rows="10">
        ...
        <!-- 비용 수정 다이얼로그 -->
        <Dialog
          class="dialog-editexpense"
          header="비용 수정하기"
          v-model:visible="displayEditExpense"
          :modal="true"
          style="width: 510px"
        >
          <div>
        ...
        <Dialog
          ...
          header="지출 통계"
          v-model:visible="displaySummary"
          :modal="true"
          style="width: 800px"
        >
          <div>
            ...
            <div>
              <h3>총 지출 금액</h3>
              <h1>
                <span style="color: #009688">{{ new Intl.NumberFormat("en-US").format(summaryData.getSummary()) }}</span
                >원
              </h1>
                ...
                <div v-for="cat of summaryData.data.labels" :key="cat">
                  ...
                  <h1 class="summary-h1">
                    ...
                    <span style="color: #009688">{{
                      new Intl.NumberFormat("en-US").format(summaryData.getCategory(cat))
                    }}</span>
                    원
                  </h1>
                </div>
            ...
            <div v-for="day of scheduleData.getDayCount() + 1" :key="day">
              ...
              <h3 v-else>day {{ day - 1 }}&nbsp;&nbsp;{{ getDateAfter(day - 2) }}</h3>
              <br />
              <DataTable class="board-table" :value="filteredExpenseData[day - 1]" responsiveLayout="scroll" :rows="10">
                <Column field="category" header="유형" style="width: 120px">
                  <template #body="slotProps">
                    <i class="pi" style="font-size: 3rem; color: #009688" :class="getIcon(slotProps.data.category)"></i>
                  </template>
        ...
        <!-- 정산 다이얼로그 -->
        <Dialog
          class="dialog-calculate"
          header="정산"
          v-model:visible="displayCalculate"
          :modal="true"
          style="width: 510px"
        >
          <div>
            ...
            <div>
              ...
              <div v-for="user in scheduleData.users" :key="user">
                <div
                  v-show="user != 'aaa' && calculateData['aaa'][user] != 0"
                ...
                >
                  ...
                  <h3>
                    <span style="color: #009688">{{
                      new Intl.NumberFormat("en-US").format(Math.abs(calculateData["aaa"][user].toFixed(0)))
                    }}</span>
                    원
                  </h3>
                </div>
            ...
            <div>
              ...
              <div v-for="user in scheduleData.users" :key="user" style="display: flex; justify-content: space-between">
                ...
                <h3>
                  <span style="color: #009688">{{
                    new Intl.NumberFormat("en-US").format(calculateData[user][user].toFixed(0))
                  }}</span>
                  원
                </h3>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      summaryData: {
        ...
        getSummary() {
          let sum = 0;
          for (let amount of this.data.datasets[0].data) sum += amount;
          return sum;
        },
  ...
  created() {
    ...
    // 지출 내역 따오기
    axios
      .get(`${this.apiURL}/${this.scheduleData.skdNum}/expense`, {
        headers: {
      ...
      .then(
        function (resp) {
          ...
          if (this.expenseData.length == 0) {
            for (let i = 0; i < this.scheduleData.getDayCount() + 1; i++) this.expenseData.push([]);
          }
  ...
  methods: {
    ...
    checkAvailable() {
      if (this.expDay == null || this.expDay.code == undefined) return { result: false, reason: "날짜를 선택하세요." };
      if (this.expContent == "") return { result: false, reason: "내용을 입력하세요." };
      if (this.expPayment == "" || this.expPayment == 0) return { result: false, reason: "금액을 입력하세요." };
      if (this.expPayType == "") return { result: false, reason: "현금/카드 여부를 선택하세요." };
      if (this.expCategory == null || this.expCategory.value == undefined)
        return { result: false, reason: "소비 카테고리를 선택하세요." };
      if (this.expConsumers.length == 0) return { result: false, reason: "함께한 사람을 최소 한 명 이상 선택하세요." };
      if (this.expPayers.length == 0) return { result: false, reason: "결제한 사람을 최소 한 명 이상 선택하세요." };
      return { result: true };
    },
    getChartData() {
      let result = {
    ...
    getFilteredExpenseData(category) {
      if (category == "전체 보기") return this.expenseData;
    ...
    calculate() {
      ...
      for (let user of this.scheduleData.users) {
        result[user] = [];
        for (let user2 of this.scheduleData.users) result[user][user2] = 0; // user가 user2에게 n원을 줘야한다는 뜻, 스스로가 스스로에게 주는 경우[user][user] 지출을 뜻함
      }
      for (let arr of this.expenseData) {
        for (let element of arr) {
          // 2안
          ...
          for (let user of element.payer) {
            // 쓴 사람 검색
            // 자신의 지출에 추가
            result[user][user] += payMoney;
            // 논 사람들에게서 돈 걷기
            for (let user2 of element.consumer) {
              // 논 사람 검색
            ...
            if (gap > 0) {
              for (let user3 of element.payer) {
                // 또! 쓴 사람 검색
